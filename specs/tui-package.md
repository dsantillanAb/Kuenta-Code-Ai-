...
`packages/kuenta-devs/src/cli/cmd/tui` into a self-contained workspace package while
...
```text
packages/tui
name: @kuenta-devs/tui
```

Target dependency graph:

```text
packages/kuenta-devs ---\
                      > @kuenta-devs/tui -> @kuenta-devs/sdk
packages/cli --------/
```
...
generic presentation libraries. It must not depend on `packages/kuenta-devs`,
`packages/cli`, or `@kuenta-devs/core`.
...
- Do not preserve private imports by creating aliases from `packages/tui` back
  into `packages/kuenta-devs`.
- Do not replace private `packages/kuenta-devs` imports with `@kuenta-devs/core`
...
- Keep legacy CLI command parsing, server startup, worker management,
  authentication, and config discovery outside `@kuenta-devs/tui`.

## Ownership Boundary

### `@kuenta-devs/tui` Owns
...
```text
packages/kuenta-devs/src/cli/cmd/tui
```

Its private dependency on `packages/kuenta-devs` is primarily expressed through
the `@/*` TypeScript alias, which resolves to `packages/kuenta-devs/src/*`.
...
Status: Completed. The private `@kuenta-devs/tui` workspace package now has an
...
- Add `packages/tui/package.json` with the name `@kuenta-devs/tui`.
...
- The package has no dependency on `opencode`, `@kuenta-devs/cli`, or
  `@kuenta-devs/core`.
...
primitives now live in `@kuenta-devs/tui`. The legacy host consumes explicit
...
and build inputs through `@kuenta-devs/tui/runtime`. Movable app, component,
...
`@kuenta-devs/tui/config` now owns schemas, defaults, keybind resolution, the
...
- Keep config path discovery, project/global precedence, migration, variable
  expansion, and plugin package installation in `packages/kuenta-devs` initially.
...
canonical in `@kuenta-devs/tui`. Configured references resolve through the new
...
`@kuenta-devs/tui`. The legacy host injects a narrow plugin host that retains
...
- Move plugin rendering tests to `packages/tui`; retain installation/loading
  integration tests in `packages/kuenta-devs`.
...
- The package has no imports from `packages/kuenta-devs`, `packages/cli`, or
  `@kuenta-devs/core`.
...
public `@kuenta-devs/tui` root while retaining worker/server/config/plugin and
...
- Keep the legacy yargs commands corresponding to current `thread.ts` and
  `attach.ts` in `packages/kuenta-devs`.
- Keep the legacy embedded worker and server startup in `packages/kuenta-devs`.
- Change those adapters to load config, create transport inputs, and call the
  public `@kuenta-devs/tui` API.
...
- Remove obsolete `@tui/*` path mappings from `packages/kuenta-devs`.
- Remove stale test fixtures and update all imports to package exports.
- Narrow `@kuenta-devs/tui` exports to intentional public entrypoints.
...
- No source under `packages/tui` imports `@/...`, `@kuenta-devs/core`, or either
  executable package.
...
cd packages/kuenta-devs && bun typecheck
...
rg '@kuenta-devs/core|packages/kuenta-devs|packages/cli' packages/tui
rg 'src/cli/cmd/tui|@tui/' packages/kuenta-devs/src packages/kuenta-devs/test
```
...
- Build the current-platform `packages/kuenta-devs` binary.

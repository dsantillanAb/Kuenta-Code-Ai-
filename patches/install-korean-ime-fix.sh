...
PROMPT_FILE="$OPENCODE_SRC/packages/kuenta-devs/src/cli/cmd/tui/component/prompt/index.tsx"
...
cd "$OPENCODE_SRC/packages/kuenta-devs"
...
BUILT_BINARY="$OPENCODE_SRC/packages/kuenta-devs/dist/opencode-${PLATFORM}-${ARCH}/bin/opencode"

if [ ! -f "$BUILT_BINARY" ]; then
  BUILT_BINARY=$(find "$OPENCODE_SRC/packages/kuenta-devs/dist" -name "opencode" -type f -executable 2>/dev/null | head -1)
fi
...
if [ -f "$BUILT_BINARY" ]; then
  ...
  echo "  cd $OPENCODE_SRC/packages/kuenta-devs && bun run build --single"
  exit 1
fi

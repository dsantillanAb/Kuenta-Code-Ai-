...
This is the living design and status document for `@kuenta-devs/codemode` and its existing V2 OpenCode adapter.
...
`packages/kuenta-devs/src/tool/code-mode.ts`, where nested MCP calls run the `tool.execute.before` and

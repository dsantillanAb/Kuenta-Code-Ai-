...
import type { AssistantMessage, Message, OpencodeClient, Session, SessionMessageResponse } from "@kuenta-devs/sdk/v2"
...
function promptErrorMessage(error: AssistantError) {
  ...
  return "Kuenta Devs prompt failed"
}
...
function fromUnknownError(error: unknown, service?: string): Error {
  ...
  return new ACPError.ServiceFailureError({ safeMessage: "Kuenta Devs service failure", service })
}

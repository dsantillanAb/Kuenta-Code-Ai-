...
import { NonNegativeInt, PositiveInt } from "@kuenta-devs/core/schema"
import { ConfigAttachmentV1 } from "@kuenta-devs/core/v1/config/attachment"
import { ConfigLSPV1 } from "@kuenta-devs/core/v1/config/lsp"
import { InvalidError } from "@kuenta-devs/core/v1/config/error"
  ...
  if (permissions.length)
    throw new InvalidError({
      ...
      issues: permissions.map((path) => ({
        path,
        message: 'V2 permissions are not supported by Kuenta Devs V1. Use V1 "permission" rules or run kuenta-devs2.',
      })),
    })

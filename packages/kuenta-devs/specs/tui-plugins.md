...
- Author package entrypoint is `@kuenta-devs/plugin/tui`.
...
- Import types from `@kuenta-devs/plugin/tui`.
- `@kuenta-devs/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@kuenta-devs/plugin/tui"
...
When `plugin` entries exist in a writable `.opencode` dir or `OPENCODE_CONFIG_DIR`, OpenCode installs `@kuenta-devs/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@kuenta-devs/plugin/tui`.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@kuenta-devs/plugin/tui`.

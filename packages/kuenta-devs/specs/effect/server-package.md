...
- The server still lives in `packages/kuenta-devs`.
...
Until enough shared service code lives outside `packages/kuenta-devs`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/kuenta-devs`

It should not import `packages/kuenta-devs` services while `packages/kuenta-devs`
...
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/kuenta-devs` implementation details.

...
Delete `packages/kuenta-devs/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/kuenta-devs/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/kuenta-devs/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/kuenta-devs`
- TUI plugin loader tests in `packages/kuenta-devs` if runtime plugin API wiring changed

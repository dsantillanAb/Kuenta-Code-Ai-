...
Trim `packages/kuenta-devs/src/server/routes/instance/httpapi/public.ts` until OpenAPI generation is mostly a direct projection of the `HttpApi` route declarations, without breaking the generated SDK surface.
...
- `[x]` Add or extend `packages/kuenta-devs/test/server/httpapi-query-schema-drift.test.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/kuenta-devs`.
- `[x]` `bun typecheck` from `packages/kuenta-devs`.

### PR 2: Delete Spec-Only Workspace Query Injection

- `[x]` Edit `packages/kuenta-devs/src/server/routes/instance/httpapi/public.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/kuenta-devs`.
- `[x]` `bun dev generate > /tmp/opencode-openapi.json` from `packages/kuenta-devs`.
...
- Edit `packages/kuenta-devs/src/server/routes/instance/httpapi/public.ts`.
...
- `bun dev generate > /tmp/opencode-openapi.json` from `packages/kuenta-devs`.
...
- Check source schemas in files like `packages/kuenta-devs/src/session/schema.ts`, `packages/kuenta-devs/src/permission/schema.ts`, and pty schema definitions.
...
- Edit route group files under `packages/kuenta-devs/src/server/routes/instance/httpapi/groups/`.
- Replace SDK-visible `HttpApiError.BadRequest` / `HttpApiError.NotFound` with explicit error schemas from `packages/kuenta-devs/src/server/routes/instance/httpapi/errors.ts` or add new ones there.
